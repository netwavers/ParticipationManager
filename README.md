...
1. **ブラウザで開く**: 
   - PC: `index.html`をブラウザで開きます
   - モバイル: `mobile.html`をブラウザで開きます（スマホ向け最適化UI）
2. **参加者を追加**: 
   - 手動で名前を入力して追加、または
...
3. **セッション開始**: 
   - セッションサイズを設定
   - タイマーを設定（オプション）
   - 「Start」または「Random」をクリック
4. **セッション管理**: 
   - プレイ終了後、チェックマークボタン（Finish）で履歴に移動
   - 必要に応じて矢印ボタン（Return to Queue）で待機キューに戻す
...
OBSなどの配信ソフトにオーバーレイとして情報を表示できます。

1. **オーバーレイの表示**:
   - 画面上部の「OBS Overlay」ボタンをクリックします。
   - ポップアップウィンドウ (`obs.html`) が開きます。

2. **OBSへの追加**:
   - OBSの「ブラウザソース」を追加します。
   - URLに `obs.html` のフルパス、またはローカルサーバーのURLを入力します。
   - 推奨サイズ: 幅 400px, 高さ 600px (レイアウトに合わせて調整してください)
   - 背景は透過されているため、配信画面に自然に馴染みます。

3. **同期**:
   - メイン画面 (`index.html`) の操作（待機列の更新、セッション開始、タイマー）はリアルタイムでオーバーレイに反映されます。

#### 方法B: OBSドック機能（背景透過したい場合）

OBSの中に操作画面 (`index.html`) を取り込むことで、ブラウザソース (`obs.html`) と通信させることができます。

1.  **操作画面の追加**:
    - OBSのメニュー「ドック」→「**カスタムブラウザドック**」を開きます。
    - ドック名: `Participation Manager`（任意）
    - URL: `index.html` のフルパス（例: `file:///D:/path/to/index.html`）
    - 「適用」を押すと、OBS内に操作パネルが表示されます。これを好きな場所に配置します。

2.  **オーバーレイの追加**:
    - OBSの「ソース」→「**ブラウザ**」を追加します。
    - URL: `obs.html` のフルパス（例: `file:///D:/path/to/obs.html`）
    - 幅: `400`, 高さ: `600`

3.  **確認**:
    - ドックに追加した操作パネルで操作すると、画面上のオーバーレイが連動して動きます。背景もきれいに透過されます。

### セッションタイマー

1. **タイマー設定**: 「Timer Duration (min)」で時間を設定（1〜60分）
